...
class TestProxyRoutingWebsite < Minitest::Test
  ...
  def test_default_website
    ...
    response = Typhoeus.get("https://127.0.0.1:9081/signup/", self.http_options.except(:headers))
    assert_equal(200, response.code, response.body)
    assert_match("API Key Signup", response.body)
  end

  def test_signup_https_redirect
    response = Typhoeus.get("http://127.0.0.1:9080/signup/", self.http_options.except(:headers))
  ...
  def test_signup_https_redirect_wildcard_host
    response = Typhoeus.get("http://127.0.0.1:9080/signup/", self.http_options.except(:headers).deep_merge({
      :headers => {
        "Host" => "unknown.foo",
      },
    }))
