...
class TestProxyFormattedErrorsTemplates < Minitest::Test
  include ApiUmbrellaTests::Setup
  include ApiUmbrellaTests::FormattedErrors
  parallelize_me!
  ...
  def test_strips_leading_and_trailing_whitespace
    ...
    prepend_api_backends([@api]) do
      ...
      response = Typhoeus.get("http://127.0.0.1:9080/#{unique_test_id}/hello.xml", self.http_options.except(:headers))
      assert_equal('<?xml version="1.0" encoding="UTF-8"?><code>API_KEY_MISSING</code>', response.body)

      response = Typhoeus.get("http://127.0.0.1:9080/#{unique_test_id}/hello.csv", self.http_options.except(:headers))
      assert_equal(%(Code\n"API_KEY_MISSING"), response.body)

      response = Typhoeus.get("http://127.0.0.1:9080/#{unique_test_id}/hello.html", self.http_options.except(:headers))
