...
class TestProxyRequestRewritingAppendQueryString < Minitest::Test
  ...
  def test_appends_to_existing_query_string
    response = Typhoeus.get("http://127.0.0.1:9080/#{unique_test_class_id}/info/?test=value", self.http_options)
    assert_equal(200, response.code, response.body)
    data = MultiJson.load(response.body)
    assert_equal({
      "test" => "value",
  ...
  def test_overrides_existing_query_string
    response = Typhoeus.get("http://127.0.0.1:9080/#{unique_test_class_id}/info/?test=value&add_param1=original", self.http_options)
    assert_equal(200, response.code, response.body)
    data = MultiJson.load(response.body)
    assert_equal({
      "test" => "value",
  ...
  def test_leaves_query_string_when_append_value_is_empty_string
    response = Typhoeus.get("http://127.0.0.1:9080/#{unique_test_class_id}/empty-append/info/?test=value", self.http_options)
    assert_equal(200, response.code, response.body)
    data = MultiJson.load(response.body)
    assert_equal({
      "test" => "value",
  ...
  def test_preserves_query_string_order
    response = Typhoeus.get("http://127.0.0.1:9080/#{unique_test_class_id}/info/?ccc=foo&aaa=bar&b=test", self.http_options)
    assert_equal(200, response.code, response.body)
    data = MultiJson.load(response.body)
