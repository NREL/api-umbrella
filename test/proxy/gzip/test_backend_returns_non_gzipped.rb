...
class Test::Proxy::Gzip::TestBackendReturnsNonGzipped < Minitest::Test
  ...
  def test_gzips_response_when_content_length_exceeds_1000
    response = Typhoeus.get("http://127.0.0.1:9080/api/compressible/1000", http_options.deep_merge(:accept_encoding => "gzip"))
  ...
  def test_does_not_gzip_response_when_content_length_less_than_1000
    response = Typhoeus.get("http://127.0.0.1:9080/api/compressible/999", http_options.deep_merge(:accept_encoding => "gzip"))
    assert_equal(200, response.code, response.body)
  ...
  def test_gzips_chunked_responses_of_any_size
    response = Typhoeus.get("http://127.0.0.1:9080/api/compressible-delayed-chunked/5", http_options.deep_merge(:accept_encoding => "gzip"))
    assert_equal(200, response.code, response.body)
  ...
  def test_returns_unzipped_when_client_does_not_support
    response = Typhoeus.get("http://127.0.0.1:9080/api/compressible/1000", http_options)
    assert_equal(200, response.code, response.body)
    refute(response.headers["content-encoding"])
    assert_equal(1000, response.body.bytesize)
  end
