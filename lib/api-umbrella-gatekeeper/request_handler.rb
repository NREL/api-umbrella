...
module ApiUmbrella
  module Gatekeeper
    ...
    #
    #     handler = ApiUmbrella::Gatekeeper::RequestHandler.new(headers)
    ...
    class RequestHandler
      ...
      def proxy_instruction
        status, headers, response = ApiUmbrella::Gatekeeper::RackApp.instance.call(self.headers)

        if(status == 200)
          { :remote => self.class.random_api_router_server }
        else
          error_response = ApiUmbrella::Gatekeeper::HttpResponse.new(status, headers, response)
