...
_.extend(Worker.prototype, {
  ...
  connectBeanstalk: function(asyncReadyCallback) {
    ...
    this.beanstalk.on('connect', function() {
      connected = true;
      this.beanstalk.use('logs', function(error) {
        asyncReadyCallback(error);
      });
    }.bind(this));
  ...
  handleSyslogMessage: function(rawMessage) {
    ...
    try {
      ...
      logger.error('JSON parse error for log message', { error: error, message: message });
      return false;
    }
    ...
    this.redis.hset('log:' + id, source, data, function(error) {
      ...
      if(source === 'initial_router') {
        ...
        this.beanstalk.put(priority, delay, ttr, id, function(error) {
          if(error) {
            logger.error('Failed to queue log for processing', { id: id, source: source, data: data, error: error });
          }
        });
      }
    }.bind(this));
