...
_.extend(DnsResolver.prototype, {
  ...
  finishInit: function(callback, error) {
    logger.info('System DNS discovered: ' + JSON.stringify(this.nameServers));
  ...
  handleNameServerChanges: function() {
    this.nameServers = dns.platform.name_servers;
    logger.info('System DNS changes detected. Using new name servers: ' + JSON.stringify(this.nameServers));
  },
  ...
  resolveHost: function(host, resolveHostCallback) {
    ...
    if(ipaddr.isValid(host)) {
      ...
      this.handleResolveHostEnd(host, resolveHostCallback);
      return false;
    }

    async.series([
      this.fetchCachedHost.bind(this, host),
      this.sendHostQuery.bind(this, host, resolveHostCallback),
    ]);
  },
  ...
  sendHostQuery: function(host, resolveHostCallback, asyncCallback) {
    ...
    request.on('end', this.handleResolveHostEnd.bind(this, host, resolveHostCallback));
  ...
  handleResolveHostEnd: function(host, resolveHostCallback) {
    ...
    if(resolved.activeIp !== resolved.existingIp) {
      this.redis.set('router_active_ip:' + host, resolved.activeIp, function(error) {
        if(error) {
          logger.error('Error setting active IP: ', error);
        }
      });
    }
  },
