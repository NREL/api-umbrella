      ...
      this.getRecentlySeenIps(host, function(error, recentIps) {
        ...
        if(resolved.activeIp) {
          ...
          if(resolved.existingIp && resolved.existingIp !== DnsResolver.NULL_ROUTE) {
            ...
            logger.warning('Unable to resolve host: ' +  host + ' (using cached IP: ' + ip + ' ' + resolved.lookupFailures + ' consecutive failures, retrying in ' + ttl + ' seconds)');
          } else if(recentIps && recentIps[0] && recentIps[0] !== DnsResolver.NULL_ROUTE) {
            ip = recentIps[0];
            logger.warning('Unable to resolve host: ' +  host + ' (using recent IP: ' + ip + ' ' + resolved.lookupFailures + ' consecutive failures, retrying in ' + ttl + ' seconds)');
          } else {
            ip = DnsResolver.NULL_ROUTE;
            logger.error('Unable to resolve host: ' +  host + ' (using null route... ' + resolved.lookupFailures + ' consecutive failures, retrying in ' + ttl + ' seconds)');
          }
