...
_.extend(DnsResolver.prototype, {
  ...
  finishInit: function(callback, error) {
    logger.info({ nameServers: this.nameServers }, 'System DNS discovered');
  ...
  handleNameServerChanges: function() {
    this.nameServers = dns.platform.name_servers;
    logger.info({ nameServers: this.nameServers }, 'System DNS changes detected. Using new name servers');
  },
  ...
  resolveHost: function(host, resolveHostCallback) {
    logger.info({ host: host }, 'Resolving host');
  ...
  fetchCachedHost: function(host, asyncCallback) {
    ...
    if(resolved.activeIp) {
      ...
      this.redis.get('router_active_ip:' + host, function(error, ip) {
        if(error) {
          logger.error({ err: error }, 'Error fetching active IP');
          asyncCallback(error);
        }
  ...
  handleResolveHostMessage: function(host, error, message) {
    ...
    message.answer.forEach(function(answer) {
      if(answer.address) {
        logger.info({ host: host, name: answer.name, address: answer.address }, 'Resolved host');
    ...
    if(!resolved || !resolved.ips || resolved.ips.length === 0) {
      ...
      nodeDns.lookup(host, function(error, ip) {
        if(!error && ip) {
          ...
          logger.error({ host: host }, 'Unable to resolve host via lookup or fallback');
        }
  ...
  setActiveIp: function(host, asyncCallback) {
    ...
    this.addRecentlySeenIps(host, resolved, function() {
      this.getRecentlySeenIps(host, function(error, recentIps) {
        ...
        if(resolved.activeIp) {
          resolved.lookupFailures = 0;
        } else {
          resolved.lookupFailures = resolved.lookupFailures || 0;
          resolved.lookupFailures++;

          var ttlBackoff = 2 * 60; // 2 minutes
          var ttlMax = 8 * 60 * 60; // 8 hours
          var ttl = Math.pow(2, (resolved.lookupFailures - 1)) * ttlBackoff;
          ttl = Math.min(ttl, ttlMax);

          var ip;
          if(resolved.existingIp && resolved.existingIp !== DnsResolver.NULL_ROUTE) {
            ip = resolved.existingIp;
            logger.warn({ host: host, ip: ip, lookupFailures: resolved.lookupFailures, retryIn: ttl }, 'Unable to resolve host - using cached IP - retrying later');
          } else if(recentIps && recentIps[0] && recentIps[0] !== DnsResolver.NULL_ROUTE) {
            ip = recentIps[0];
            logger.warn({ host: host, ip: ip, lookupFailures: resolved.lookupFailures, retryIn: ttl }, 'Unable to resolve host - using recent IP - retrying later');
          } else {
            ip = DnsResolver.NULL_ROUTE;
            logger.warn({ host: host, ip: ip, lookupFailures: resolved.lookupFailures, retryIn: ttl }, 'Unable to resolve host - using null route - retrying later');
          }
  ...
  finishResolveHost: function(host, resolveHostCallback) {
    ...
    if(resolved.activeIp !== resolved.existingIp) {
      logger.info({ host: host, oldIp: resolved.existingIp, newIp: resolved.activeIp }, 'IP change for host');
    ...
    if(resolved.ttl) {
      logger.info({ host: host, activeIp: resolved.activeIp, refreshIn: resolved.ttl }, 'Scheduling refresh of host');
    ...
    if(resolved.activeIp !== resolved.existingIp) {
      this.redis.set('router_active_ip:' + host, resolved.activeIp, function(error) {
        if(error) {
          logger.error({ err: error }, 'Error caching active IP');
        }
      });
    }
  },
  ...
  addRecentlySeenIps: function(host, resolved, callback) {
    ...
    logger.info({ host: host, ignoreTtl: ignoreTtl, ips: ips }, 'Setting host IPs');
    this.redis.multi()
      .sadd(setKey, ips)
      .expire(setKey, 5 * 60 * 60) // 5 hours
      .exec(function(error) {
        if(error) {
          logger.error({ err: error }, 'Error setting recently seen ips');
        }
  ...
  getRecentlySeenIps: function(host, callback) {
    ...
    async.times(2, function(i, setCallback) {
      ...
      this.redis.smembers(setKey, function(error, ips) {
        if(error) {
          logger.error({ err: error }, 'Error fetching recently seen ips');
          setCallback(null, []);
          return false;
        }
      ...
      if(allIps && allIps.length > 0) {
        logger.info({ host: host, ips: ips }, 'Recently cached IPs');
      }
  ...
  getIp: function(host) {
    ...
    if(this.resolved[host]) {
      ...
      if(!ip && this.resolved[host].existingIp) {
        ...
        logger.warn({ host: host, ip: ip }, 'Unexpectedly using existing IP for host');
      }
    }

    if(!ip) {
      ip = DnsResolver.NULL_ROUTE;
      logger.warn({ host: host, ip: ip }, 'Unexpectedly fetching IP for unknown host - using null route');
    }
