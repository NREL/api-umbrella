...
var _ = require('lodash'),
    async = require('async'),
    config = require('api-umbrella-config').load(),
...
_.extend(DnsResolver.prototype, {
  ...
  resolveAllHosts: function(callback) {
    ...
    console.info('Resolving all: ', apis);
    console.info('Resolving all: ', config.getAll());
  ...
  resolveHost: function(host, resolveHostCallback) {
    console.debug('Resolving host: ', host);
  ...
  handleResolveHostMessage: function(host, error, message) {
    ...
    message.answer.forEach(function(answer) {
      if(answer.address) {
        console.debug('Resolved host: ', host, answer.name, answer.address);
        ...
        if(!resolved.activeIp) {
          if(recentIps && recentIps.length > 0) {
            ...
            if(resolved.existingIp && resolved.existingIp !== DnsResolver.NULL_ROUTE) {
              console.error('Unable to resolve host: ' +  host + ' (using cached IP: ' + resolved.existingIp + ')');
              resolved.activeIp = resolved.existingIp;
            } else {
  ...
  finishResolveHost: function(host, resolveHostCallback) {
    ...
    if(resolved.ttl) {
      console.debug('Scheduling refresh of ' + host + ' (' + resolved.activeIp + ') in ' + resolved.ttl + ' seconds');
  ...
  addRecentlySeenIps: function(host, resolved, callback) {
    ...
    console.debug('Set: ', host, ignoreTtl, ips);
    this.redis.multi()
      .sadd(setKey, ips)
  ...
  getRecentlySeenIps: function(host, callback) {
    ...
    async.times(4, function(i, setCallback) {
      ...
      if(allIps && allIps.length > 0) {
        console.debug('Recently cached IPs: ', host, ips);
      }
  ...
  getIp: function(host) {
    ...
    if(!ip) {
      console.error('Unexpectedly fetching IP for unknown host: ', host);
