...
_.extend(DnsResolver.prototype, {
  ...
  initialize: function(configReloader, initCallback) {
    this.configReloader = configReloader;
    this.resolveAllHosts(initCallback);
  },

  resolveAllHosts: function(initCallback) {
    ...
    async.eachLimit(hosts, 5, this.resolveHost.bind(this), this.handleResolveAllHosts.bind(this, initCallback));
  },

  handleResolveAllHosts: function(initCallback, error) {
    initCallback(error, this);
  },
  ...
  sendHostQuery: function(host, asyncCallback) {
    ...
    request.on('timeout', function(error) {
      logger.error({ err: error, host: host }, 'Resolve host: timeout');
    });
  ...
  handleResolveHostMessage: function(host, resolved, error, message) {
    ...
    if(message.header.rcode === ndns.consts.NAME_TO_RCODE.NOTFOUND) {
  ...
  setResolvedHost: function(host, resolved, asyncCallback) {
    ...
    if(resolved.servers.length === 0) {
      logger.error({ host: host, existing: this.resolved[host] }, 'Resolve host: Lookup unexpectedly failed, keeping existing');
    } else {
      if(!_.isEqual(this.resolved[host], resolved)) {
  ...
  scheduleHostRefresh: function(host, asyncCallback) {
    ...
    if(!resolved || !resolved.isIp) {
      ...
      this.resolveHostTimeouts[host] = setTimeout(this.resolveHost.bind(this, host), ttl * 1000);
    }
  ...
  getServers: function(host) {
    ...
    if(!servers || servers.length === 0) {
      ...
      logger.error({ host: host, servers: servers }, 'Unexpectedly fetching servers for unknown host - using null route');
    }
