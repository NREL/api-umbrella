...
_.extend(DnsResolver.prototype, {
  ...
  finishInit: function(callback, error) {
    console.info('System DNS discovered: ', this.nameServers);
  ...
  handleNameServerChanges: function() {
    this.nameServers = dns.platform.name_servers;
    console.info('System DNS changes detected. Using new name servers: ', this.nameServers);
  },
  ...
  resolveHost: function(host, resolveHostCallback) {
    console.info('Resolving host: ', host);
  ...
  fetchCachedHost: function(host, asyncCallback) {
    ...
    if(resolved.activeIp) {
      ...
      this.redis.get('router_active_ip:' + host, function(error, ip) {
        if(error) {
          console.error('Error fetching active IP: ', error);
          asyncCallback(error);
        }
  ...
  handleResolveHostMessage: function(host, error, message) {
    ...
    message.answer.forEach(function(answer) {
      if(answer.address) {
        console.info('Resolved host: ', host, answer.name, answer.address);
    ...
    if(!resolved || !resolved.ips || resolved.ips.length === 0) {
      ...
      nodeDns.lookup(host, function(error, ip) {
        if(!error && ip) {
          ...
          console.error('Unable to resolve host via lookup or fallback: ' +  host);
        }
      ...
      this.getRecentlySeenIps(host, function(error, recentIps) {
        ...
        if(!resolved.activeIp) {
          ...
          if(resolved.existingIp && resolved.existingIp !== DnsResolver.NULL_ROUTE) {
            ...
            console.error('Unable to resolve host: ' +  host + ' (using cached IP: ' + ip + ')');
          } else if(recentIps && recentIps[0] && recentIps[0] !== DnsResolver.NULL_ROUTE) {
            ip = recentIps[0];
            console.error('Unable to resolve host: ' +  host + ' (using recent IP: ' + ip + ')');
          } else {
            ip = DnsResolver.NULL_ROUTE;
            console.error('Unable to resolve host: ' +  host + ' (using null route...)');
          }
  ...
  finishResolveHost: function(host, resolveHostCallback) {
    ...
    if(resolved.activeIp !== resolved.existingIp) {
      console.info('IP change for ' + host + ': ' + resolved.existingIp + ' => ' + resolved.activeIp);
    ...
    if(resolved.ttl) {
      console.info('Scheduling refresh of ' + host + ' (' + resolved.activeIp + ') in ' + resolved.ttl + ' seconds');
    ...
    if(resolved.activeIp !== resolved.existingIp) {
      this.redis.set('router_active_ip:' + host, resolved.activeIp, function(error) {
        if(error) {
          console.error('Error caching active IP: ', error);
        }
      });
    }
  },
  ...
  addRecentlySeenIps: function(host, resolved, callback) {
    ...
    console.info('Set: ', host, ignoreTtl, ips);
    this.redis.multi()
      .sadd(setKey, ips)
      .expire(setKey, 5 * 60 * 60) // 5 hours
      .exec(function(error) {
        if(error) {
          console.error('Error setting recently seen ips:', error);
        }
  ...
  getRecentlySeenIps: function(host, callback) {
    ...
    async.times(2, function(i, setCallback) {
      ...
      this.redis.smembers(setKey, function(error, ips) {
        if(error) {
          console.error('Error fetching recently seen ips:', error);
          setCallback(null, []);
          return false;
        }
      ...
      if(allIps && allIps.length > 0) {
        console.info('Recently cached IPs: ', host, ips);
      }
  ...
  getIp: function(host) {
    ...
    if(this.resolved[host]) {
      ...
      if(!ip && this.resolved[host].existingIp) {
        ...
        console.warn('Unexpectedly using existing IP for host: ' +  host + ' (using existing ip: ' + ip + ')');
      }
    }

    if(!ip) {
      ip = DnsResolver.NULL_ROUTE;
      console.error('Unexpectedly fetching IP for unknown host: ' +  host + ' (using null route: ' + ip + ')');
    }
