...
_.extend(Worker.prototype, {
  ...
  handleConnections: function(error) {
    if(error) {
      logger.error({ err: error }, 'Distributed rate limits sync connections error');
      process.exit(1);
      return false;
    }

    this.refreshCollections();
    config.on('change', this.refreshCollections.bind(this));
  ...
  processSyncQueue: function(rateLimit, mongoResult, callback) {
    this.redis.get(mongoResult._id, function(error, redisCount) {
      if(error) {
        logger.error({ err: error }, 'Distributed rate limits sync Redis result error');
        callback(error);
        return false;
      }
