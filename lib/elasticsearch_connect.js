...
_.extend(ElasticSearchConnect.prototype, {
  initialize: function(callback) {
    ...
    this.client = new elasticsearch.Client(this.config.get('elasticsearch'));
  ...
  setupDefaultAliases: function(asyncReadyCallback) {
    var env = this.config.get('environment');
    ...
    var aliases = _.uniq([
      {
        ...
        index: 'api-umbrella-logs-' + this.config.get('log_template_version') + '-' + env + '-' + today,
      },
      {
      ...
      },
      {
        ...
        index: 'api-umbrella-logs-' + this.config.get('log_template_version') + '-' + env + '-' + tomorrow,
      },
      {
