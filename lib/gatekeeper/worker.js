...
_.extend(Worker.prototype, {
  ...
  handleConnections: function(error) {
    if(error) {
      logger.error({ err: error }, 'Gatekeeper worker connections error');
      process.exit(1);
      return false;
    }
  ...
  handleProxyError: function(error, request, response) {
    ...
    // Adjust the log level down for socket hang up issues that are likely
    // caused by the 60 second request time out.
    var logLevel = 'error';
    var errorMessage = error;
    if(error && error.message === 'socket hang up' && responseTime && responseTime > 55000) {
      logLevel = 'info';
      errorMessage = error.message + ' (likely due to request timeout)';
    }

    logger[logLevel]({ err: error, req: request, responseTime: responseTime }, 'Proxy error: ' + errorMessage);
