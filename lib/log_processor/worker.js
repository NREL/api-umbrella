    ...
    // retried later.
      ...
      this.getFailedJobAttempts(jobId, logId, function(error, failedAttempts) {
        ...
        if(failedAttempts && failedAttempts >= 3 && log && log.initial_router) {
          logger.info({ logId: logId, jobId: jobId, failedAttempts: failedAttempts, hasInitialRouterData: !!log.initial_router, hasGatekeeperData: !!log.gatekeeper, hasApiBackendRouterData: !!log.api_backend_router }, 'Log data incomplete - logging anyway');
          callback(null, log);
        } else {
