    ...
    // 5, then 10 seconds.
      ...
      this.beanstalk.stats_job(jobId, function(error, stats) {
        ...
        if(failedAttempts >= 2) {
          ...
          logger.warn({ err: error, logId: logId, jobId: jobId, failedAttempts: failedAttempts, delay: delay }, 'Log data incomplete - scheduling retry');
  ...
  rescheduleFailedJob: function(jobId, logId, callback) {
    this.beanstalk.stats_job(jobId, function(error, stats) {
      ...
      if(failedAttempts > 10) {
        logger.error({ err: error, logId: logId, jobId: jobId, failedAttempts: failedAttempts }, 'Log processing failed too many times - burying permanently');
        ...
        logger.warn({ err: error, logId: logId, jobId: jobId, failedAttempts: failedAttempts, delay: delay }, 'Log processing failed - scheduling retry');
