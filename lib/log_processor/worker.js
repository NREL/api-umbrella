...
// Override Redis convoy's createClient method so we can connect to the correct
// redis db (otherwise convoy just connects to the default redis port and
// there's no way to configure it).
Convoy.redis.createClient = function(){
  return redis.createClient(config.get('redis.port'), config.get('redis.host'));
};
...
_.extend(Worker.prototype, {
  ...
  handleLogCleaned: function(id, done, error, log) {
    var index = 'api-umbrella-logs-write-' + config.get('app_env') + '-' + moment(log.request_at).utc().format('YYYY-MM');
