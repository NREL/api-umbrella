...
_.extend(Gatekeeper.prototype, {
  ...
  startMaster: function() {
    MongoClient.connect(config.get('mongodb'), function(error, db) {
      this.configBuilder = new ConfigBuilder(db, this.handleConfigReady.bind(this));
    }.bind(this));
  },

  handleConfigReady: function() {
    if(this.configOverrides) {
      config.updateRuntime({ apiUmbrella: this.configOverrides });
    }
  ...
  closeNonForked: function(callback) {
    ...
    if(this.configBuilder) {
      closeFunctions.push(this.configBuilder.close.bind(this.configBuilder));
    }
...
exports.startNonForked = function(configOverrides, readyCallback) {
  var gatekeeper = new Gatekeeper(readyCallback);
  gatekeeper.configOverrides = configOverrides;
