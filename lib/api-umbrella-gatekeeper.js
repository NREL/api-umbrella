...
_.extend(Gatekeeper.prototype, {
  ...
  startHaproxyLogListener: function(callback) {
    this.routerLogListenerWorker = new routerLogListener.Worker(this);
    if(callback) {
  ...
  forkProxyWorkers: function(startupCallback) {
    var numWorkers = parseInt(this.config.get('proxy:workers'), 10);
  ...
  forkHaproxyLogListenerWorkers: function(startupCallback) {
    var numWorkers = parseInt(this.config.get('router_log_listener:workers'), 10);
    async.times(numWorkers, function(index, forkCallback) {
  ...
  forkLogProcessorWorkers: function(startupCallback) {
    var numWorkers = parseInt(this.config.get('log_processor:workers'), 10);
    async.times(numWorkers, function(index, forkCallback) {
  ...
  handleHaproxyLogWorkerExit: function(code, signal) {
    ...
    new routerLogListener.Worker(this);
  },
  ...
  initConfig: function(configOverrides) {
    ...
    this.config.file('defaults', path.join(configDir, 'default.json'));

    this.config.file('internal_defaults', path.resolve(__dirname, '../config/default' + '.json'));

    // Set the global 'geodatadir' variable geoip-lite optionally reads from.
    var geoipDataDir = this.config.get('geoip_data_dir');
    if(geoipDataDir) {
