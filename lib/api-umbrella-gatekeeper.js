...
_.extend(Gatekeeper.prototype, {
  ...
  startWorkers: function() {
    ...
    case 'log_processor':
      ...
      break;
    default:
      logger.warn('Unexpected worker type: ' + process.env.GATEKEEPER_WORKER_TYPE);
      break;
    }
  },
  ...
  handleProxyWorkerExit: function(code) {
    logger.warn('proxy worker ' + process.pid + ' died ('+code+'). restarting...');
    new proxy.Worker();
  },

  handleHaproxyLogWorkerExit: function(code) {
    logger.warn('router log listener worker ' + process.pid + ' died ('+code+'). restarting...');
    new routerLogListener.Worker();
  },

  handleLogWorkerExit: function(code) {
    logger.warn('log processor worker ' + process.pid + ' died ('+code+'). restarting...');
