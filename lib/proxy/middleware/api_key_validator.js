...
_.extend(ApiKeyValidatorRequest.prototype, {
  initialize: function(validator, request, response, next) {
    ...
    if(apiKey) {
      ...
      this.response.end(i18n.__('api_key_none', config.get('accountSignupUri')));
    }
  },
  ...
  handleUser: function(error, user) {
    if(user) {
      if(!user.disabled_at) {
        ...
        this.response.statusCode = 403;
        this.response.end(i18n.__('api_key_disabled', config.get('contactUri')));
      }
    } else {
      this.response.statusCode = 403;
      this.response.end(i18n.__('api_key_invalid', config.get('accountSignupUri')));
    }
  },
});
