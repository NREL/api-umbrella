...
module.exports = function rateLimit(server, timeWindowOptions) {
  var middleware = new RateLimit(server, timeWindowOptions);
...
_.extend(RateLimit.prototype, {
  initialize: function(server, timeWindowOptions) {
    this.server = server;

    this.timeWindows = timeWindowOptions.map(function(options) {
      return new TimeWindow(this.server, options);
    }.bind(this));
  },
...
_.extend(TimeWindow.prototype, {
  initialize: function(server, options) {
    ...
    if(options.distributed) {
      this.mongoCollection  = this.server.mongo.collection('rate_limits_' + this.prefix.replace(/:/, '_'));
...
_.extend(RateLimitRequest.prototype, {
  ...
  finished: function(error, limitRequests) {
    if(error) {
      ...
      limitRequests.forEach(function(limitRequest) {
        limitRequest.increment();
      });
    }
  }
});
...
_.extend(RateLimitRequestTimeWindow.prototype, {
  initialize: function(rateLimitRequest, timeWindow, asyncRequestCallback) {
    ...
    this.server = rateLimitRequest.rateLimit.server;
  ...
  performRedisRateLimit: function(asyncFetchCallback) {
    this.server.redis.mget(this.getDurationBucketKeys(), this.handleRedisResponse.bind(this, asyncFetchCallback));
  },
  ...
  incrementMongo: function() {
    this.timeWindow.mongoCollection.update({
      ...
      { "$inc": { count: 1 } },
      { upsert: true },
      this.handleIncrementMongo.bind(this));
  },
  ...
  handleDistributedSync: function(error) {
    ...
    if(this.mongoSum > this.redisSum) {
      ...
      this.server.redis.incrby(this.getCurrentBucketKey(), difference);
    }
  },
  ...
  getDurationBucketKeys: function() {
    if(!this.durationBucketKeys) {
      this.durationBucketKeys = this.getDurationBucketDates().map(function(date) {
        return this.getKey() + ":" + date.toISOString();
      }.bind(this));
    }
