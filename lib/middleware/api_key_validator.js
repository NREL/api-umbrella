...
module.exports = function apiKeyValidator(server) {
  var middleware = new ApiKeyValidator(server);
...
_.extend(ApiKeyValidator.prototype, {
  initialize: function(server) {
    this.server = server;
    this.users = this.server.mongo.collection('api_users');
    this.apiKeyMethods = this.server.config.get('api_key_methods');
  },
...
_.extend(ApiKeyValidatorRequest.prototype, {
  initialize: function(validator, request, response, next) {
    ...
    if(apiKey) {
      ...
      this.response.end(i18n.__('api_key_none', this.server.config.get('account_signup_uri')));
    }
  },
  ...
  handleUser: function(error, user) {
    if(user) {
      if(!user.disabled_at) {
        ...
        this.response.statusCode = 403;
        this.response.end(i18n.__('api_key_disabled', this.server.config.get('contact_uri')));
      }
    } else {
      this.response.statusCode = 403;
      this.response.end(i18n.__('api_key_invalid', this.server.config.get('account_signup_uri')));
    }
  },
});
