...
module.exports = function roleValidator(server) {
  var middleware = new RoleValidator(server);
...
_.extend(RoleValidator.prototype, {
  initialize: function(server) {
    this.server = server;
    this.restrictedApis = this.server.config.get("restricted_apis");
  },

  handleRequest: function(request, response, next) {
    var requiredRoles = [];
    if(this.restrictedApis) {
      this.restrictedApis.forEach(function(api) {
    ...
    if(authenticated) {
      ...
      response.end(i18n.__('api_key_unauthorized', this.server.config.get('contact_uri')));
    }
  }
});
