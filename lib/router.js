...
_.extend(Router.prototype, {
  ...
  writeTemplates: function(callback) {
    var templateConfig = _.extend({}, this.config.getAll(), {
      ...
      primary_hosts: _.filter(this.config.get('hosts'), function(host) { return !host.secondary; }),
      secondary_hosts: _.filter(this.config.get('hosts'), function(host) { return host.secondary; }),
    });
  ...
  writeStaticSiteEnv: function(callback) {
    var envPath = path.join(this.config.get('static_site.dir'), '.env');
    var envLines = [];
      ...
      fs.writeFile(envPath, envLines.join('\n'), function(error) {
        ...
        if(this.config.get('app_env') === 'development') {
          ...
          execFile('bundle', ['exec', 'middleman', 'build'], {
            cwd: this.config.get('static_site.dir'),
            env: processEnv.env,
          }, callback);
        }
      }.bind(this));
