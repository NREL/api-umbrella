...
_.extend(Router.prototype, {
  ...
  reload: function() {
    ...
    execFile('supervisorctl', ['-c', configPath, 'update'], execOpts, function() {
      async.parallel([
        function(callback) {
          execFile('supervisorctl', ['-c', configPath, 'kill', 'HUP', 'nginx_router'], execOpts, callback);
        },
        function(callback) {
          execFile('supervisorctl', ['-c', configPath, 'kill', 'HUP', 'web_nginx'], execOpts, callback);
        },
        function(callback) {
          execFile('supervisorctl', ['-c', configPath, 'kill', 'USR2', 'web_puma'], execOpts, callback);
        },
        function(callback) {
          execFile('supervisorctl', ['-c', configPath, 'serialrestart', 'gatekeeper:*'], execOpts, callback);
        },
        function(callback) {
          execFile('supervisorctl', ['-c', configPath, 'restart', 'config_reloader', 'distributed_rate_limits_sync', 'log-processor', 'router-log-listener'], execOpts, callback);
        },
      ], function(error) {
        if(error) {
