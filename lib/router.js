...
_.extend(Router.prototype, {
  initialize: function(options) {
    ...
    this.supervisordConfig = path.resolve(__dirname, '../config/supervisord.conf'),
    this.processEnv = {
      'PATH': [
  ...
  handleConfigReady: function(error, configLoader) {
    ...
    this.processEnv['API_UMBRELLA_CONFIG'] = this.configLoader.runtimeFile;
  ...
  reload: function() {
    var supervisorctl = 'supervisorctl -c ' + this.supervisordConfig;
    var execOpts = {
      env: this.processEnv,
    };

    exec(supervisorctl + ' update', execOpts, function() {
      async.parallel([
        function(callback) {
          exec(supervisorctl + ' kill HUP nginx_router', execOpts, callback);
        },
        function(callback) {
          exec(supervisorctl + ' kill HUP web_nginx', execOpts, callback);
        },
        function(callback) {
          exec(supervisorctl + ' kill USR2 web_puma', execOpts, callback);
        },
        function(callback) {
          exec(supervisorctl + ' serialrestart gatekeeper:*', execOpts, callback);
        },
        function(callback) {
          exec(supervisorctl + ' restart config_reloader distributed_rate_limits_sync log-processor router-log-listener', execOpts, callback);
        },
      ]);
    }.bind(this));
  },
  ...
  startSupervisor: function(callback) {
    this.supervisordProcess = new (forever.Monitor)(['supervisord', '-c', this.supervisordConfig, '--nodaemon'], {
      max: 1,
      silent: true,
