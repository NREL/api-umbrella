...
_.extend(Router.prototype, {
  ...
  writeTemplates: function(callback) {
    ...
    glob(path.join(templateRoot, '**/*'), function(error, templatePaths) {
      ...
      templatePaths = _.reject(templatePaths, function(p) { return _.contains(p, 'nginx/api_backends.conf'); });
  ...
  waitForConnections: function(callback) {
    ...
    if(_.contains(config.get('services'), 'general_db')) {
      listeners = listeners.concat([
        'tcp://127.0.0.1:' + config.get('mongodb.embedded_server_config.port'),
      ]);
    }

    if(_.contains(config.get('services'), 'log_db')) {
      listeners = listeners.concat([
    ...
    if(_.contains(config.get('services'), 'web')) {
      listeners = listeners.concat([
