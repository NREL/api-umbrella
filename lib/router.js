...
_.extend(Router.prototype, {
  initialize: function(options) {
    ...
    process.on('SIGUSR1', this.reopenLogs.bind(this));
  ...
  reopenLogs: function() {
    ...
    if(config.get('service_general_db_enabled')) {
      tasks = tasks.concat([
        function(callback) {
          logger.info('Reopening logs for mongod...');
          execFile('supervisorctl', ['-c', configPath, 'kill', 'USR1', 'mongod'], execOpts, callback);
        },
      ]);
    }
    ...
    if(tasks.length > 0) {
      async.parallel(tasks, function(error) {
        if(error) {
          logger.error('Error reopening log files: ', error);
          return false;
        }
  ...
  checkFreeMemory: function(callback) {
    ...
    if(freeMem < preferredFree) {
      ...
      if(freeSwap < preferredSwap) {
        var warning = 'System with low memory does not have at least 1GB of free swap (' + freeSwap + ' MB available). API Umbrella may fail to start.';
        logger.warning(warning);
        if(config.get('app_env') === 'test') {
          console.warn('\nWARNING: ' + warning);
        }
        ...
        if(posix.geteuid() !== 0) {
          try {
            ...
            if(newLimit > existingLimits.hard) {
              ...
              // positives during the tests, if we can't achieve the levels of
              // concurrency we expect.
              if(config.get('app_env') === 'test') {
                console.warn('\nWARNING: ' + warning);
              }
