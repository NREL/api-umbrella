...
_.extend(Router.prototype, {
  ...
  writeTemplates: function(callback) {
    var templateConfig = _.extend({}, config.getAll(), {
      ...
      mongodb_config: _.merge({
        fork: 'false',
        logpath: path.join(config.get('log_dir'), 'mongod.log'),
      ...
      elasticsearch_yaml: yaml.safeDump(_.merge({
        ...
        cluster: {
          name: 'api-umbrella-' + config.get('app_env'),
        },
    ...
    async.each(listeners, function(listener, eachCallback) {
      ...
      async.until(function() {
        ...
        switch(parsed.protocol) {
          case 'http:':
            request({ url: parsed.href, timeout: 1500 }, function(error, response) {
