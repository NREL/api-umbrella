...
_.extend(Router.prototype, {
  initialize: function(options) {
    ...
    apiUmbrellaConfig.loader({
      defaults: {
        ...
        trafficserver_run_dir: runDir + '/trafficserver',
      },
      paths: [
  ...
  handleConfigReady: function(error, configLoader) {
    ...
    async.series([
      ...
      this.installTrafficServerConfig.bind(this),
  ...
  installTrafficServerConfig: function(callback) {
    // Create the directories TrafficServer needs to start in our custom run
    // directory. Since we're starting trafficserver up with a custom TS_ROOT,
    // trafficserver needs this directory structure (and there's no real other
    // way to configure it). 
    ['etc/trafficserver', 'var/trafficserver', 'var/log/trafficserver'].forEach(function(dir) {
      fsExtra.mkdirpSync(path.join(this.config.get('trafficserver_run_dir'), dir));
    }.bind(this));

    // Copy our config files into the custom run directory's config dir.
    glob(this.config.get('config_dir') + '/trafficserver/*.config', function(error, configPaths) {
      async.each(configPaths, function(configPath, eachCallback) {
        var destPath = path.join(this.config.get('trafficserver_run_dir'), 'etc/trafficserver', path.basename(configPath));
        fsExtra.copy(configPath, destPath, eachCallback);
      }.bind(this), callback);
    }.bind(this));
  },
  ...
  waitForConnections: function(callback) {
    var listeners = [
      ...
      'tcp://127.0.0.1:' + this.config.get('trafficserver.port'),
