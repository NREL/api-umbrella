...
_.extend(Router.prototype, {
  initialize: function(options) {
    ...
    var supervisord = spawn('supervisord', ['-c', 'config/supervisord.conf', '--nodaemon'], {
      env: {
        'PATH': path.resolve(__dirname, '../gatekeeper/bin') + ':/opt/api-umbrella/embedded/elasticsearch/bin:/opt/redis/bin:/opt/nodejs/bin:/opt/nginx/sbin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin',
      }
    });
