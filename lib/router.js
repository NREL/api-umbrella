...
_.extend(Router.prototype, {
  ...
  reload: function() {
    ...
    execFile('supervisorctl', ['-c', configPath, 'update'], execOpts, function(error, stdout, stderr) {
      if(error) {
        console.error('supervisorctl update error: ' + error.message + '\n\nSTDOUT: ' + stdout + '\n\nSTDERR:' + stderr);
        return false;
      }
  ...
  writeTemplates: function(callback) {
    var gatekeeperPorts = _.times(config.get('gatekeeper.workers'), function(n) {
      return parseInt(config.get('gatekeeper.starting_port'), 10) + n;
    });
    ...
    var templateConfig = _.extend({}, config.getAll(), {
      ...
      gatekeeper_hosts: _.map(gatekeeperPorts, function(port) {
        return config.get('gatekeeper.host') + ':' + port;
      }),
      ...
      gatekeeper_ports: gatekeeperPorts,
      gatekeeper_supervisor_process_names: _.map(gatekeeperPorts, function(port) {
        return 'gatekeeper_' + port;
      }),
