  ...
  seedDatabase: function(callback) {
    async.series([
      function(seriesCallback) {
        var key = 'api-umbrella-web-backend';
        Api.findOne({ key: key }, function(error, api) {
          if(error) { return seriesCallback(error); }

          if(!api) {
            api = new Api({ _id: uuid.v4(), key: key });
          }

          api.set({
            name: 'API Umbrella - Default',
            frontend_host: this.config.get('default_host'),
            backend_host: this.config.get('default_host'),
