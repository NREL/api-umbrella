...
_.extend(Router.prototype, {
  initialize: function(options) {
    ...
    process.on('SIGTERM', this.stop.bind(this, function() {
      process.exit(0);
    }));
  ...
  handleConfigReady: function(error, configLoader) {
    ...
    async.series([
      ...
      this.testEnvMongoOrchestration.bind(this),
      this.waitForConnections.bind(this),
      this.testEnvWaitForMongoOrchestration.bind(this),
    ...
    if(config.get('service_router_enabled')) {
      tasks = tasks.concat([
        function(callback) {
          ...
          supervisorSignal('router-nginx', 'SIGUSR1', callback);
        },
        function(callback) {
          logger.info('Reopening logs for varnishncsa...');
          supervisorSignal('varnishncsa', 'SIGHUP', callback);
        },
        function(callback) {
          logger.info('Reopening logs for dnsmasq...');
          supervisorSignal('dnsmasq', 'SIGUSR2', callback);
        },
      ]);
    }
    ...
    if(config.get('service_web_enabled')) {
      tasks = tasks.concat([
        function(callback) {
          ...
          supervisorSignal('web-nginx', 'SIGUSR1', callback);
        },
      ]);
    }
  ...
  internalConfigDefaults: function(callback) {
    ...
    if(defaultHost && defaultHost.hostname) {
      defaultHostname = defaultHost.hostname;
    }
      ...
      internal_website_backends: [
        {
          ...
          frontend_host: defaultHostname,
          backend_protocol: 'http',
          server_host: config.get('static_site.host'),
          server_port: config.get('static_site.port'),
        },
      ],
    });
  ...
  checkFreeMemory: function(callback) {
    meminfo(function(error, data) {
      var preferredFree = 1.5 * 1024; // 1.5 GB

      if(error) {
        console.warn('\nWARNING: Could not determine amount of free memory. You may experience degraded performance if this system does not have at least ' + preferredFree + ' MB free. (Error: ' + error + ')');
        return callback();
      }

      // Get the real amount of free memory, plus buffers/cached.
      var freeMem = Math.round(parseInt(data.MemFree, 10) / 1024 + parseInt(data.Buffers, 10) / 1024 + parseInt(data.Cached, 10) / 1024);
      if(freeMem < preferredFree) {
        process.stderr.write('\n');
        var warning = 'System does not have at least ' + preferredFree + ' MB of free memory (' + freeMem + ' MB available). You may experience degraded performance.';
        console.warn('\nWARNING: ' + warning);

        var freeSwap = Math.round(parseInt(data.SwapFree, 10) / 1024);
        var preferredSwap = 1 * 1024;
        if(freeSwap < preferredSwap) {
          warning = 'System with low memory does not have at least ' + preferredSwap + ' MB of free swap (' + freeSwap + ' MB available). API Umbrella may fail to start.';
          console.warn('\nWARNING: ' + warning);
        }
      }
  ...
  startSupervisor: function(callback) {
    this.supervisordProcess = new (forever.Monitor)(['supervisord', '-c', processEnv.supervisordConfigPath(), '--nodaemon'], {
      ...
      // Kill supervisor with SIGTERM signal instead of default SIGKILL, so
      // supervisor can more cleanly exit all of the child processes.
      killSignal: 'SIGTERM',
      killTTL: 10000,
      killTree: true,
    });
  ...
  testEnvMongoOrchestration: function(callback) {
    if(config.get('app_env') !== 'test') {
      return callback();
    }

    var timeout = setTimeout(function() {
      callback('Unable to establish connection to mongo-orchestration');
    }.bind(this), 15000);
    timeout.unref();
    ...
    async.until(function() {
      ...
      request.put({
        ...
        body: fs.readFileSync(path.resolve(__dirname, '../test/config/mongo-orchestration.json')).toString(),
        timeout: 120000,
      }, function(error) {
        if(error) {
        ...
        this.mongoOrchestrationComplete = true;
      }.bind(this));
  ...
  testEnvWaitForMongoOrchestration: function(callback) {
    if(config.get('app_env') !== 'test') {
      return callback();
    }

    async.until(function() {
      return this.mongoOrchestrationComplete;
    }.bind(this), function(untilCallback) {
