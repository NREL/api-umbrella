...
local function bucket_keys(user, limit, current_time)
  ...
  for i = 0, num_buckets - 1 do
    table.insert(bucket_keys, key_base .. ":" .. (bucket_time - limit["accuracy"] * i))
  end

  return bucket_keys
end

local function increment_limit(current_time_key, duration)
...
local function get_remaining_for_limit(settings, user, limit, current_time)
  ...
  for index, key in ipairs(keys) do
    ...
    -- ngx.log(ngx.ERR, "CURRENT_TIME: " .. inspect(current_time) .. " LIMIT: " .. inspect(limit["limit"]) .. " BUCKET: " .. inspect(key) .. " BUCKET COUNT: " .. inspect(bucket_count) .. " REMAINING: " .. inspect(remaining))
...
local function is_over_any_limits(settings, user, current_time)
  ...
  for _, limit in ipairs(limits) do
    if not over_limit or limit["response_headers"] then
      ...
      if remaining then
        --ngx.log(ngx.ERR, "LIMIT: " .. inspect(limit) .." REMAINING: " .. inspect(remaining))
        if remaining < 0 then
          over_limit = true
          remaining = 0
        end
...
local function increment_all_limits(limits)
  for _, limit in ipairs(limits) do
    increment_limit(limit["_current_time_key"], limit["duration"])
  end
end
...
return function(settings, user)
  ...
  if over_limit then
    ...
    increment_all_limits(settings["rate_limits"])
  end
end
