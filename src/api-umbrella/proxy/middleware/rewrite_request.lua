...
local function pass_api_key(ngx_ctx, settings)
  ...
  if pass_api_key_header and api_key then
    ...
    req_set_header("X-Api-Key", api_key)
  else
    req_clear_header("X-Api-Key")
  end
  ...
  if api_key and ngx_ctx.remote_user == api_key then
...
local function set_user_id_header(user)
  if user then
    req_set_header("X-Api-User-Id", user["id"])
  else
    req_clear_header("X-Api-User-Id")
  end
end

local function set_roles_header(user)
  if user and user["roles"] then
    req_set_header("X-Api-Roles", table.concat(keys(user["roles"]), ","))
  else
    req_clear_header("X-Api-Roles")
  end
end

local function append_query_string(ngx_ctx, settings)
...
local function set_headers(settings)
  ...
  if settings["_headers"] then
    for _, header in ipairs(settings["_headers"]) do
      ...
      req_set_header(header["key"], value)
    end
  end
end
...
local function set_http_basic_auth(settings)
  if settings["_http_basic_auth_header"] then
    req_clear_header("Authorization")
    req_set_header("X-Api-Umbrella-Backend-Authorization", settings["_http_basic_auth_header"])
  end
end

local function strip_cookies(api)
  local cookie_header = ngx_var.http_cookie
  ...
  if #kept_cookies == 0 then
    req_clear_header("Cookie")
  else
    req_set_header("Cookie", table.concat(kept_cookies, "; "))
  end
end

local function url_rewrites(ngx_ctx, api)
  if not api["rewrites"] then return end

  local request_method = ngx_ctx.request_method
  local original_uri = ngx_ctx.request_uri
  local new_uri = ngx_ctx.request_uri
  ...
  if new_uri ~= original_uri then
    ...
    set_uri(ngx_ctx, path, args)
  end
end

return function(ngx_ctx, user, api, settings)
  pass_api_key(ngx_ctx, settings)
  set_user_id_header(user)
  set_roles_header(user)
  append_query_string(ngx_ctx, settings)
  set_headers(settings)
  set_http_basic_auth(settings)
  strip_cookies(api)
  url_rewrites(ngx_ctx, api)
end
