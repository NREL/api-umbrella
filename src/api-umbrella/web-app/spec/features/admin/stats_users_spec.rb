...
RSpec.describe "analytics by users", :js => true do
  login_admin
  ...
  describe "xss" do
    it "escapes html entities in the table" do
      ...
      FactoryGirl.create(:xss_log_item, :request_at => Time.parse("2015-01-16T06:06:28.816Z").utc, :api_key => user.api_key, :user_id => user.id, :user_email => user.email, :user_registration_source => user.registration_source)
